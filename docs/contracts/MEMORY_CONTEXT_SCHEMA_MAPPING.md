...
| M-03 Historical statement never automatically User-Provided; only Category A may become a cited Inference premise; Category B restricted to DISCOVERY_ATTENTION; neither becomes Assumed | Not enforceable at this layer | This invariant is primarily about `IntentSpec`'s own Claim-origin behavior when (once dependency B exists) it cites a `MemoryContext` entry — behavior this artifact cannot observe. Partial guard: Category A/B are structurally distinguished (see M-18) and `SEMANTIC_PREMISE` is validator-restricted to Category A entries only (see M-11/M-19), narrowing what a future citation could even legitimately point at. |
...
| M-08 Memory alone never closes a HIGH/CRITICAL item | Not applicable to this artifact | Concerns `IntentSpec`'s own Decision Impact handling of a downstream item once dependency B exists. `MemoryContext` itself carries no Decision Impact concept at all. |
...
| M-14 Production record must preserve the named audit facts | Schema-enforced | M-14 lists these facts for retrieved memories generally, not only admitted ones, so `excluded_entries` carries the same depth as `admitted_entries`: retrieved-memory identity (`brain_memory_id`), an explicit outcome discriminator (`retrieval_outcome`, three closed values), content retained as an actual copy for both admitted and excluded entries (`content`, plus canonical `brain_memory_id` — Brain returns full record content for anything retrieved, whether ultimately admitted or excluded), retrieval time distinct from Brain's own timestamps (`retrieval_time` vs. `brain_created_at`/`brain_updated_at`), the retrieval query and purpose (`retrieval_query`, `retrieval_purpose`), each entry's scope and provenance (`source.scope`, `source.provenance`), the authority classification with its basis/method/ambiguity where one was actually attempted (`classification.*`, nullable on an excluded entry when exclusion happened before any classification was attempted, e.g. `inbox`-type or a mechanical scope mismatch — see M-06/M-16), the freshness judgment (`freshness.*`, nullable for the same reason), why admitted or excluded, and which stage was authorized to use it (`consuming_stage`). For an admitted entry, "why admitted" is deliberately not a separate free-text field: the entry's own required fields (it cleared the scope check, carries no disqualifying `brain_status`/`superseded_by`, and received a classification and tier) already are the admission rationale, and a redundant restatement would only duplicate them. For an excluded entry, no such implicit union of fields exists — `exclusion_reason` is required precisely because the other fields may legitimately be absent (`classification`/`freshness` null). Whether a recorded rationale honestly reflects the real reason, rather than a fabricated one, is Not enforceable at this layer. |
...
| M-20 Historical force never mechanically copied into current Inferred Claim force | Not applicable to this artifact | Concerns a downstream `IntentSpec` Inferred Claim's own force-derivation step at Intent Parsing (once dependency B exists), not anything `MemoryContext` itself represents. This artifact carries no normative-force field for any entry. |
| M-21 DECISION_OPTION supplies zero independent authority; currently named only for a memory-informed R-19 default | Not applicable to this artifact, partially covered elsewhere | The R-09/R-19 justification-and-citation discipline this invariant describes belongs to Requirement Derivation (dependency D), a downstream stage this task does not touch. The one portion this schema *can*, and does, enforce — `DECISION_OPTION` is categorically unavailable to any historical-user-statement entry, of either category — is validator-enforced and already counted under M-11's row. |
...
at all); and a `lesson`-typed entry whose content inspection reclassifies it as a Category A
historical user statement reaching `SEMANTIC_PREMISE` — the type-independence guarantee extended to
`lesson`/`playbook`, not only to `reference`/`pattern`/`incident`/`project`.
...
historical) not restricted to `PROCESS_ONLY`; `DECISION_OPTION` claimed for a historical-user-
statement entry; an admitted entry carrying `brain_status: "superseded"`; an admitted entry carrying
a non-null `superseded_by` despite an `active` `brain_status`; a Category A claim with no
`historical_citation`; an admitted entry whose `scope` does not match `run_context`; and an ambiguous
heuristic classification claiming `DECISION_OPTION` despite the fail-closed rule.
