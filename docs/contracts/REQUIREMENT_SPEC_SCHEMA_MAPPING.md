...
Requirement-Level-Inference clause premised on a conditional claim or clause — see §8.
...
`oneOf` with exactly three members — `{kind: "claim", claim_id}`, `{kind: "requirement", requirement_id}`,
and `{kind: "requirement_clause", requirement_id, clause_id}` — and no fourth shape exists anywhere in
...
**R-22 × mixed-strength Requirement (task Section 16) — resolved as a representation refinement, not
new semantics; both premise shapes coexist, one is not a replacement for the other.** An earlier draft
of this schema offered *only* `requirement_clause` as the premise shape, removing R-10's own literal
"the accepted Claim or **Requirement** it derives from" option entirely — an independent review pass
correctly flagged this as itself an undisclosed narrowing serious enough to warrant reconsidering
whether a `SCHEMA_DESIGN_GAP` stop was the right call. The fix restores `{kind: "requirement",
requirement_id}` as the direct representation of R-10's literal premise unit — validator-checked to
have a **single strength across every one of its clauses** (`requirement-spec-r15-rli-whole-requirement-premise.json`;
`requirement-spec-r16-rli-whole-requirement-mixed-strength.json` proves a mixed-strength Requirement is
rejected here, with the error naming the narrower shape as the correct alternative) — undefined
otherwise, exactly the case "Requirement Cardinality and Granularity" already establishes is undefined
at the whole-Requirement level. `{kind: "requirement_clause", requirement_id, clause_id}` remains the
**narrower escape valve**, naming one specific clause so R-22's strength inheritance is still
deterministic when the premise Requirement's clauses genuinely disagree in strength — a representation
refinement of "Requirement Cardinality and Granularity"'s already-established per-clause strength
independence, not a new rule, and now additive to the whole-Requirement shape rather than a replacement
for it. `requirement-spec-t-rli-from-requirement-clause.json` demonstrates the clause-level valid case;
`requirement-spec-w-r22-strength-inflation.json` and `requirement-spec-x-r22-strength-weakening.json`
demonstrate the validator's exact-equality enforcement in both directions, for both premise shapes.

**RLI premise on an invalidated Requirement/clause is rejected.** "Requirement Invalidation and
Re-Derivation" treats an inference whose premise no longer holds as itself unsupported; the validator
checks the premise Requirement's (or premise clause's owning Requirement's) `status !== "invalidated"`
for all three premise shapes (`requirement-spec-r14-rli-on-invalidated-requirement.json`).
...
premise clause's/Requirement's own) carries one — otherwise a condition preserved through a
...
"requirement_clause"` directly, and from `premise.kind === "requirement"` fanned out to every clause of
the premise Requirement) must be acyclic — validator-enforced with the same visiting/visited DFS
...
| R-09 RD-introduced default + R-24 citation distinctly labeled | Schema + Validator-enforced | `working_defaults[]` structurally requires `rationale` and keeps `memory_informed_rationale` a separate, optional, distinctly-shaped field (schema); the memory citation, when present, is validator-resolved against a companion `MemoryContext` (§9); `source_open_item_id` uniqueness document-wide is validator-enforced so one Unknown cannot be filled twice, contradictorily. |
| R-10 RLI states premise/reasoning, distinctly labeled | Schema + Validator-enforced | `requirement_level_inference` is a wholly separate clause shape from `direct_compilation` (never folded together); premise resolution, including for a whole-Requirement premise, is validator-enforced (§8). |
...
| R-12 Identity persists; revision supersedes, never mutates in place | Schema-enforced (identity/version fields; `revision` is schema-required, present-but-nullable, closing a prior omitted-key gap — §10) + Not enforceable at this layer (non-mutation proof requires cross-version storage comparison). |
| R-13 Supersession requires reconsideration | Validator-enforced (structural: `revision` required non-null at `version > 1`, and required to name at least one affected Requirement/Open Item — §10) + Not enforceable at this layer (whether reconsideration was substantively correct for each affected Requirement). |
| R-14 Invalidated marked, never deleted | Schema + Validator-enforced | `invalidation` is schema-required (present-but-nullable, §10); the validator requires it non-null exactly when `status: "invalidated"` and requires a `reason` when present. Nothing in this schema can delete a prior array element from a *new* version's document (a new version is simply a new document). |
...
| R-17 Empty Requirement set, Complete, is valid | Schema-enforced | No `minItems` on `requirements`. |
...
| R-22 RLI strength = premise strength exactly | Validator-enforced | Exact equality against the resolved premise's strength, whichever of the three premise shapes is used: a Claim (mapped force), a whole Requirement (single strength across all its clauses, or rejected as undefined), or one specific clause (§8). Also rejects a premise on an invalidated Requirement/clause. |
| R-23 RLI premise never MemoryContext | Schema-enforced | `requirementLevelInferencePremise`'s closed `oneOf` has no shape for a `MemoryContext` reference — structurally impossible, not merely unchecked (§8). |
| R-24 Memory-informed rationale (Gate 2, provenance-additional, no voting, one fill per Unknown) | Validator-enforced | The most extensively validator-enforced invariant in this document — see §9 for the full checklist, including the now-mandatory `requirement_derivation`-stage `upstream_artifact_binding` and the document-wide one-fill-per-Unknown rule. |
...
- **Post-review regression fixtures (`requirement-spec-r1`–`r19-*.json`)**: added after four independent
  ...
  memory citation required to trigger it (R13, §9); an RLI premised on an invalidated Requirement's
  clause (R14, §8); the restored whole-Requirement RLI premise, both its valid and its
